...
export class JobService {
 constructor({dir,key,fetchImpl=fetch,base='https://api-eu-central-1-dc8.poixe.com',textModel='gpt-5.2',imageModel='gemini-2.5-flash-image'}){Object.assign(this,{dir,key,fetchImpl,base,textModel,imageModel});this.jobs=new Map();this.running=false;this.closed=false;}
 ...
 async persist(job){const temp=join(this.dir,`${job.id}.${randomUUID()}.tmp`);await writeFile(temp,JSON.stringify(job),{mode:0o600});await rename(temp,join(this.dir,job.id+'.json'));}
 ...
 const j={id:data.id,owner,hash,input,state:'QUEUED',stage:'排队等待',createdAt:Date.now(),deadline:Date.now()+30*60*1000,textModel:this.textModel,imageModel:this.imageModel,artDirection:ART_DIRECTION_VERSION,pages:[],attempts:[],consentVersion:'local-ai-test-v1',consentAt:new Date().toISOString()};
 ...
 async recover(j){const a=j.pending;if(!a)return null;this.active(j);j.stage='正在查询中断响应';await this.persist(j);
 ...
 j.pending.result=result;await this.persist(j);return result;
 }
 async invoke(j,stage,path,body){this.active(j);
 if(j.pending){if(j.pending.stage!==stage)throw new Error('任务阶段不一致，已停止。');if(j.pending.result)return j.pending.result;try{const result=await this.recover(j);if(result)return result}catch{}this.active(j);j.state='WAITING_RECOVERY';j.stage='连接中断，等待找回';j.error='结果可能已生成或计费。点击“找回已有结果”，不会重新生成。';await this.persist(j);throw new Error('WAITING');}
 const limit=4;if(j.attempts.length>=limit)throw new Error(`已达到本次 ${limit} 次模型调用上限。`);
 const a={stage,status:'submitted',startedAt:Date.now(),body};j.pending=a;j.attempts.push(a);j.stage=stage;await this.persist(j);
 try{const {json,requestId}=await this.transport(path,body);this.active(j);a.status='received';a.requestId=requestId;a.usage=json.usage||json.usageMetadata;a.elapsedMs=Date.now()-a.startedAt;a.result=json;await this.persist(j);return json;
 }catch(e){if(e.message==='STOPPED')throw e;this.active(j);a.elapsedMs=Date.now()-a.startedAt;a.status=e.httpStatus||'unknown';if(e.definite){delete j.pending;throw e;}
 try{const found=await this.recover(j);if(found)return found}catch{}this.active(j);j.state='WAITING_RECOVERY';j.stage='等待找回已有结果';j.error='模型请求未完整返回，结果与计费未知。仅找回，不自动重试。';await this.persist(j);throw new Error('WAITING');}
 }
 consume(j){if(j.pending){delete j.pending.body;delete j.pending.result;delete j.pending;}}
 ...
 while(j.pages.length<3){this.active(j);const n=j.pages.length,p=j.plan.pages[n];const parts=[{text:continuationPrompt(j,n)},{inlineData:{mimeType:j.input.image.slice(5,j.input.image.indexOf(';')),data:j.input.image.split(',')[1]}}];
 ...
 const result=await this.invoke(j,`生成第 ${n+2} 页续画`,`/v1beta/models/${j.imageModel}:generateContent`,body);
 ...
 j.pages.push({text:p.text,image:`data:${image.mimeType};base64,${image.data}`});this.consume(j);await this.persist(j);}
 this.active(j);j.state='READY';j.stage='技术测试结果已生成（未人工审核）';await this.persist(j);
 }catch(e){if(['STOPPED','WAITING'].includes(e.message))return;if(!['DELETED','CANCELLED'].includes(j.state)){j.state='FAILED';j.stage='生成停止';j.error=e.message.replaceAll(this.key||'__none__','[redacted]');delete j.pending;await this.persist(j);}}
 }
 ...
 async resume(id,token){const j=this.get(id,token);if(j.state!=='WAITING_RECOVERY')return this.view(j);j.state='QUEUED';await this.persist(j);this.pump();return this.view(j);}
 async remove(id,token,state='DELETED'){const j=this.get(id,token);j.state=state;j.stage=state==='DELETED'?'已删除':'已取消';delete j.input;delete j.pending;delete j.plan;j.pages=[];j.attempts=[];await this.persist(j);return {id,state};}
}
